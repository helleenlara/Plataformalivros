...
# Dicionário para armazenar usuários (substitua por um banco de dados em produção!)
usuarios = {}
...
if __name__ == "__main__":
    st.title("Cadastro")
    cadastro_username = st.text_input("Novo Nome de Usuário")
    cadastro_password = st.text_input("Nova Senha", type="password")
    if st.button("Cadastrar"):
        if cadastrar_usuario(cadastro_username, cadastro_password):
    ...
    if st.button("Entrar"):
        if verificar_login(login_username, login_password):
            st.success("Login realizado com sucesso!")
            main()  # Chama a função principal do formulário
        else:
